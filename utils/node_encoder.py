...
def encode_nodes(nodes_to_encode: list,
                 mp: Multiplex,
                 out_dim: int = 1024,
                 base_encode_method: str = 'rwr',
                 up_encode_method: str = 'pad',
                 down_encode_method: str = None):
  ...
  if base_encode_method not in allowed_base_encode_methods:
    raise ValueError(f'base_encode_method must be {get_allowed_values_as_str(allowed_base_encode_methods)}')
  if up_encode_method not in allowed_up_encode_method:
    raise ValueError(f'up_encode_method must be {get_allowed_values_as_str(allowed_up_encode_method)}')
